...
export async function issueSession(id, role) {
  ...
  (await cookies()).set('offframe_session', token, { httpOnly: true, secure: process.env.NEXT_PUBLIC_BASE_URL?.startsWith('https://') || process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 43200, path: '/' });
}
...
export function checkOrigin(request) {
  if (!isAllowedOrigin(request, { appOrigin: process.env.NEXT_PUBLIC_BASE_URL || '', production: process.env.NODE_ENV === 'production' })) {
